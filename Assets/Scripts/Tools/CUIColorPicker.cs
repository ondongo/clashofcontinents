...
public class CUIColorPicker : MonoBehaviour
{
    ...
    private static bool GetLocalMouse(GameObject go, out Vector2 result)
    {
        ...
        var mp = rt.InverseTransformPoint(DevelopersHub.ClashOfWhatecer.CameraController.instanse.inputs.Main.PointerPosition.ReadValue<Vector2>());
    ...
    private void Setup(Color inputColor)
    {
        ...
        Action idle = () =>
        {
            if (DevelopersHub.ClashOfWhatecer.CameraController.instanse.inputs.Main.Move.WasPressedThisFrame()/*Input.GetMouseButtonDown(0)*/)
            {
                Vector2 mp;
        ...
        dragH = () =>
        {
            ...
            if (DevelopersHub.ClashOfWhatecer.CameraController.instanse.inputs.Main.Move.WasReleasedThisFrame()/*Input.GetMouseButtonUp(0)*/)
            {
                _update = idle;
            }
        };
